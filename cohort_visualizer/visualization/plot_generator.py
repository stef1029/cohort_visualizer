...
class PlotGenerator:
    ...
    def create_phase_distribution_barplot(self, output_path=None, show=False):
        ...
        try:
            # Try to convert Phase to numeric without using deprecated 'errors' parameter
        ...
        # Fix deprecated seaborn barplot usage
        ax = sns.barplot(x="Phase", y="Sessions", data=df, hue="Phase", palette=colors, legend=False)
    ...
    def create_mouse_progression_plot(self, mouse_id, output_path=None, show=False):
        ...
        try:
            df["phase_numeric"] = pd.to_numeric(df["phase"])
        except (ValueError, TypeError):
